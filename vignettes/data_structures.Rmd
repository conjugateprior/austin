...
A `jl_df` object is a `tibble` whose rows are documents and which has
a unique identifier column called `doc_id`. It may also contain 
...
In this respect
it is like a {quanteda} `corpus` with `docvars` and unlike a {tidytext} structure
because it is document-centric not token-centric. However, it reverses the 
